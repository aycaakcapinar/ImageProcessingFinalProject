...
def lip(faces_landmarks, mask_lips, image_hsv, lip_color):
...
def glasses(faces_landmarks, image_glasses, glass_color):
    global image
...
def main(image, colors):
    #image = cv2.imread("girl.jpg", cv2.IMREAD_COLOR)

    image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    mask = np.zeros_like(image)
    image_hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    image_glasses = cv2.imread("glasses.png", cv2.IMREAD_UNCHANGED)[:, :, 3]
    ...
    lip_color = colors[0]
    glass_color = colors[1]
    lip(landmarks, mask, image_hsv, lip_color)
    # Put glasses on
    glasses(landmarks, image_glasses, glass_color)
